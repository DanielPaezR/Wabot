...
def procesar_opcion_menu(numero, opcion, negocio_id):
    ...
    if opcion == '1':
        ...
        # No crear conversación aquí, dejar que mostrar_citas_para_cancelar lo haga
...
def continuar_conversacion(numero, mensaje, negocio_id):
    """Continuar conversación basada en el estado actual - CORREGIDA COMPLETA"""
    clave_conversacion = f"{numero}_{negocio_id}"
    ...
    try:
        if estado == 'solicitando_nombre':
            ...
            # ✅ NUEVO: Para cancelar citas
            return procesar_telefono_para_cancelar_citas(numero, mensaje, negocio_id)
...
def procesar_telefono_para_cancelar_citas(numero, mensaje, negocio_id):
    """Procesar teléfono ingresado para cancelar citas"""
    clave_conversacion = f"{numero}_{negocio_id}"
    ...
    print(f"🔧 [DEBUG] Teléfono válido para cancelar citas: {telefono}")
    ...
    # Intentar obtener nombre del cliente de BD
    try:
        nombre_cliente = db.obtener_nombre_cliente(telefono, negocio_id)
        if nombre_cliente and len(str(nombre_cliente).strip()) >= 2:
            conversaciones_activas[clave_conversacion]['cliente_nombre'] = str(nombre_cliente).strip()
    ...
    # Ahora mostrar citas para cancelar
    return mostrar_citas_para_cancelar(numero, negocio_id)
...
def enviar_recordatorio_24h(cita):
    """Enviar recordatorio 24 horas antes de la cita - CON SMS"""
    try:
        print(f"🔔 [SMS] Recordatorio 24h para cita #{cita.get('id')}")
        
        # SMS para el cliente
        mensaje_cliente = (
            f"Recordatorio: Tienes una cita mañana {cita.get('fecha')} "
            f"a las {cita.get('hora')} con {cita.get('profesional_nombre')} "
            f"para {cita.get('servicio_nombre')}. "
            f"Negocio: {cita.get('negocio_nombre')}"
        )
        
        # Asumiendo que tienes el teléfono del cliente en la cita
        telefono_cliente = cita.get('cliente_telefono')  # Asegúrate de incluir este campo
        
        if telefono_cliente:
            enviar_sms(telefono_cliente, mensaje_cliente)
        
        # SMS para el profesional
        mensaje_profesional = (
            f"Recordatorio: Tienes una cita mañana {cita.get('fecha')} "
            f"a las {cita.get('hora')} con {cita.get('cliente_nombre')} "
            f"para {cita.get('servicio_nombre')}."
        )
        
        # Necesitarías obtener el teléfono del profesional de tu base de datos
        # Esto es un ejemplo - ajusta según tu estructura
        telefono_profesional = cita.get('profesional_telefono')
        
        if telefono_profesional:
            enviar_sms(telefono_profesional, mensaje_profesional)
...
def enviar_recordatorio_1h(cita):
    """Enviar recordatorio 1 hora antes de la cita - CON SMS"""
    try:
        print(f"🔔 [SMS] Recordatorio 1h para cita #{cita.get('id')}")
        
        # SMS para el cliente
        mensaje_cliente = (
            f"Recordatorio: Tienes una cita en 1 hora ({cita.get('hora')}) "
            f"con {cita.get('profesional_nombre')} para {cita.get('servicio_nombre')}. "
            f"Negocio: {cita.get('negocio_nombre')}"
        )
        ...
        mensaje_profesional = (
            f"Recordatorio: Tienes una cita en 1 hora ({cita.get('hora')}) "
            f"con {cita.get('cliente_nombre')} para {cita.get('servicio_nombre')}."
        )
...
def enviar_confirmacion_cita(cita):
    """Enviar confirmación cuando se crea una cita"""
    try:
        print(f"📧 Confirmación de cita #{cita.get('id')}")
        
        # SMS para el cliente
        mensaje_cliente = (
            f"✅ Cita confirmada: {cita.get('fecha')} a las {cita.get('hora')} "
            f"con {cita.get('profesional_nombre')} para {cita.get('servicio_nombre')}. "
            f"Precio: ${cita.get('precio')}. "
            f"Recibirás recordatorios 24h y 1h antes."
        )
        ...
        # SMS para el profesional
        mensaje_profesional = (
            f"Nueva cita: {cita.get('fecha')} {cita.get('hora')} "
            f"con {cita.get('cliente_nombre')} para {cita.get('servicio_nombre')}. "
            f"Duración: {cita.get('duracion')} minutos."
        )
